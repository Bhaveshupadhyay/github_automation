...
class SlackTelemetryService(ITelemetryService):
    """Service handling real-time Slack progress telemetry with serialized non-blocking async dispatch."""
    ...
    def __init__(self, config: WorkflowEnvironment, min_update_interval: float = 2.5):
        ...
        # Start background serialized dispatcher thread
        self._worker_thread = threading.Thread(
            target=self._worker_loop,
            daemon=True,
            name="SlackTelemetryDispatcher"
        )
        self._worker_thread.start()

    def _worker_loop(self):
        """Dedicated background thread handling serialized and coalesced updates to Slack."""
        while not self._stop_event.is_set():
            self._wake_event.wait(timeout=self.min_update_interval)
            ...
            with self._lock:
                if self._is_dirty and self.state.message_ts:
                    now = time.time()
                    if self._force_next or (now - self.state.last_update_time >= self.min_update_interval):
                        payload_to_send = {
            ...
            if payload_to_send:
                self._send_slack_api("chat.update", payload_to_send)
    ...
    def flush(self) -> None:
        """Flushes any pending updates directly to Slack synchronously before process exit."""
        payload_to_send = None
        with self._lock:
            if self.state.message_ts:
                payload_to_send = {
                    "channel": self.state.channel,
                    "ts": self.state.message_ts,
                    "text": self._render_progress_text(),
