...
class SlackHistoryService:
    ...
    def fetch_first_message_text(self) -> Optional[str]:
        """Fetches Turn 1 (the initial root user message) of the Slack thread for fast target repo extraction."""
        ...
        url = f"https://slack.com/api/conversations.replies?channel={channel}&ts={thread_ts}&limit=1"
        ...
        try:
            with urllib.request.urlopen(req, timeout=5) as resp:
                ...
                if data.get("ok") and "messages" in data and len(data["messages"]) > 0:
                    first_msg = data["messages"][0].get("text", "").strip()
                    logger.info(f"💬 Retrieved root message from Slack thread {thread_ts}: '{first_msg[:60]}...'")
                    return first_msg
        except Exception as e:
            logger.warning(f"⚠️ Failed to fetch root Slack message: {e}")
