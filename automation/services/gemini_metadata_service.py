...
# google-genai's HttpOptions.timeout is in milliseconds; 15.0 there meant 15 ms, so every
# SDK call timed out and the PR fell back to a generic title.
GEMINI_SDK_TIMEOUT_MS = 30_000
GEMINI_REST_TIMEOUT_SECONDS = 30

MAX_TITLE_CHARS = 72


def fallback_pr_title(user_prompt: str) -> str:
    """A plain conventional-commit title from the request, for when the model writes none.

    Uses the request's first sentence ("add admin panel to this. it should ..." becomes
    "feat: add admin panel to this"), cut at a word boundary to fit a PR title.
    """
    text = re.sub(r"@[\w-]+(\s+AI)?", "", user_prompt or "")
    text = re.sub(r"\btarget repo:\s*\S+", "", text, flags=re.IGNORECASE)
    first = re.split(r"(?<=[.!?])\s|\n", text.strip(), maxsplit=1)[0]
    first = re.sub(r"\s+", " ", first).strip().rstrip(".!?:;,")
    if not first:
        return "feat: apply requested changes"
    first = first[0].lower() + first[1:]

    prefix = "feat: "
    budget = MAX_TITLE_CHARS - len(prefix)
    if len(first) > budget:
    ...
    def generate_metadata(self) -> GitPRDetails:
        ...
        if api_key:
            ...
            try:
                ...
                with urllib.request.urlopen(req, timeout=GEMINI_REST_TIMEOUT_SECONDS) as resp:
        ...
        commit_msg = fallback_pr_title(self.config.user_prompt)
        pr_body = (
